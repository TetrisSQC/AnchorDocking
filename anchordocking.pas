...
uses
  ...
  AnchorDockStr, AnchorDockStorage, AnchorDockPanel, anchorbutton;
...
type
  ...
  private
    ...
    procedure UpdateMinimizeButton;
  protected
...
begin
  ...
  if not (Control.AnchorSide[Side].Control is TAnchorDockSplitter) then begin
    Splitter:=nil;
    exit;
  end;
...
function TAnchorDockMaster.RestoreLayout(Tree: TAnchorDockLayoutTree;
  ...
  begin
    ...
    if ANode.NodeType=adltnControl then begin
      ...
      DisableControlAutoSizing(AControl);
      SetupSite(TWinControl(AControl),ANode,nil);
      Result:=AControl;
      // restore docked site
      if ANode.Count>0 then
        Restore(ANode[0],TWinControl(AControl));
    end
...
begin
  Result:=nil;
  if assigned(OnCreateControl) then
...
begin
  for i:=0 to ComponentCount-1 do begin
    if not (Components[i] is TAnchorDockHostSite) then continue;
...
function GetParentFormOrDockPanel(Control: TControl; TopForm:Boolean=true): TWinControl;
var
  oldControl: TControl;
begin
  oldControl:=Control;
  ...
  if Control is TCustomForm then
    ...
    Result := TAnchorDockPanel(Control)
  else
    Result := nil;
...
procedure TAnchorDockHeader.UpdateMinimizeButton;
var
  HeaderParent:TAnchorDockHostSite;
begin
  TWinControl(HeaderParent):=Parent;
